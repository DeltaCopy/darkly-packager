...
sha256_sum_tar=$(curl -Ls ${repo}/archive/refs/tags/v${version}.tar.gz | sha256sum | awk {'print $1'})
sha256_sum_zst=$(curl -Ls ${repo}/releases/download/v${version}/darkly-${version}-x86_64.pkg.zst | sha256sum | awk {'print $1'})
